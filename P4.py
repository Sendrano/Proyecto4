...
# ---
# 
# * Estudiante: **Anderson Siézar Juárez**
# * Carné: **B77360**
# * Grupo: **1**
# 
# ---
...
# 4.1 Modulación QPSK (50%). Realice una simulación del sistema de comunicaciones como en la sección 3.2.,pero utilizando
# una modulación QPSK en lugar de una modulación BPSK. Deben mostrarse las imágenes enviadas y recuperadas y las
# formas de onda.
...
def modulador(bits, fc, mpp):
    ...
    # Se proceden a sumar las portadoras en una sola variable

    Portadora_Final = portadora1 + portadora2
    
    return senal_Tx, Pm, Portadora_Final, moduladora  
...
def demodulador(senal_Rx, Portadora_Final, mpp):
    '''Un método que simula un bloque demodulador
    de señales, bajo un esquema BPSK. El criterio
    de demodulación se basa en decodificación por 
    detección de energía.
    ...
    # Energía de un período de la portadora
    Es = np.sum(Portadora_Final**2)

    # Demodulación
    for i in range(N):
        # Producto interno de dos funciones
        producto = senal_Rx[i*mpp : (i+1)*mpp] * Portadora_Final
        senal_demodulada[i*mpp : (i+1)*mpp] = producto
        Ep = np.sum(producto) 
...
# 3. Modular la cadena de bits usando el esquema BPSK
senal_Tx, Pm, Portadora_Final, moduladora = modulador(bits_Tx, fc, mpp)

# 4. Se transmite la señal modulada, por un canal ruidoso
senal_Rx = canal_ruidoso(senal_Tx, Pm, SNR)

# 5. Se desmodula la señal recibida del canal
bits_Rx, senal_demodulada = demodulador(senal_Rx, Portadora_Final, mpp)
